...
- **Consolidated master data** ⚑ — can we have **one chart of accounts, one customer list, and one item/product catalog shared across all companies** in the instance? Or is each company siloed with its own COA / customers / items?
- If master data is **siloed per company**, what's the supported workflow to **move a customer from one company to another** (e.g. when an org that used to contract with Refuel Properties now contracts with BCT)? Is it a copy-with-new-ID, or is there a transfer primitive that preserves history and external references?
- If siloed, same question for **products/items** — if we define a "Desk" item in Refuel and need the same SKU available in BCT, is that a re-create or can it be linked?
...
- [ ] Confirmed whether COA / customers / items can be **shared** across all companies — or are siloed per company
- [ ] If siloed — workflow to **move a customer** between companies documented (with external-ID preservation)
- [ ] If siloed — workflow to **share/link items** across companies documented
